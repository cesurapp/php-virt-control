<?php
	class Libvirt {
		...
		function __construct($uri = false, $debug=false, $lang=false) {
			if ($debug)
		...
		function get_cdrom_stats($domain, $sort=true) {
			...
			for ($i = 0; $i < $disks['num']; $i++) {
				$tmp = ($disks[$i] != 'hda') ? libvirt_domain_get_block_info($dom, $disks[$i]) : '';
				if ($tmp) {
					$tmp['bus'] = $buses[$i];
					$ret[] = $tmp;
				}
		...
		function domain_get_info($name = false, $name_override = false) {
			...
			$domkey  = $name_override ?? $this->domain_get_name($name);
